...
if __name__ == "__main__":
    ...
    data = np.load('prj.npy').astype('float32')[:,64:]#[0:6025]
    theta = np.load('theta.npy').astype('float32')#[0:2010]
    # Model parameters
    [ntheta,nz,n] = data.shape
    print(data.shape)
    center = 148  # rotation center
    ...
    with tc.SolverTomo(theta, ntheta, nz, n, pnz, center) as tslv:
        ...
        with dc.SolverDeform(ntheta, nz, n) as dslv:
            ...
            for k in range(niter):
                ...
                psi1 = dslv.cg_shift(data, psi1, flow, 1,
                                     tslv.fwd_tomo_batch(u)+lamd1/rho1, rho1)
                # regularization subproblem                                     
                psi2 = tslv.solve_reg(u,lamd2,rho2,alpha)    
                # tomo subproblem
                u = tslv.cg_tomo_batch_ext2(psi1-lamd1/rho1, u, 4, rho2/rho1, psi2-lamd2/rho2)
                ...
                if(np.mod(k, 4) == 0):  # check Lagrangian
                    ...
                    dxchange.write_tiff_stack(
                        u.real,  'tmp'+'_'+str(ntheta)+'_'+str(alpha)+'/rect'+str(k)+'/r', overwrite=True)
                    dxchange.write_tiff_stack(
                        psi1.real, 'tmp'+'_'+str(ntheta)+'_'+str(alpha)+'/psir'+str(k)+'/r',  overwrite=True)
                    np.save('tmp'+'_'+str(ntheta)+'_'+str(alpha)+'/flow'+str(k), flow)                # Updates
